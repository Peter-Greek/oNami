...
- **Study sessions are in-memory only**: `AppServices.sessions` is a `Map` of `StudySessionRuntime`; sessions are not persisted and are lost on app restart (or deck reset).
